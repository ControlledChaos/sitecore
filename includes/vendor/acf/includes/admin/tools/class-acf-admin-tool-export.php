	...
	function html() {
		...
		if ( ! acf_get_field_groups() ) {
			...
			printf(
				...
				__( 'There are no field groups to export. If there are field groups registered via PHP then they must first be synced to the database.', 'sitecore' ),
				admin_url( 'edit.php?post_type=acf-field-group&post_status=acfe-local' ),
				__( 'Find Field Groups', 'sitecore' )
			);
			return;
		}
